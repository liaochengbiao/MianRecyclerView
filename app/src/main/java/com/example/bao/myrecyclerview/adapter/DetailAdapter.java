package com.example.bao.myrecyclerview.adapter;
...
import com.example.bao.myrecyclerview.R;
import com.example.bao.myrecyclerview.bean.BeanDetail;
import com.example.bao.myrecyclerview.bean.BeanImage;
import com.example.bao.myrecyclerview.bean.BeanOther;
import com.example.bao.myrecyclerview.bean.BeanProduct;
import com.example.bao.myrecyclerview.utils.GlideRequestOptions;
...
public class DetailAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
            ...
            gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                ...
                public int getSpanSize(int position) {
                    if (ITEM_PRODUCT == getItemViewType(position)) {
                        //商品
                        //返回2 就是每2个占一个列  就是原本的6列就只占其中的三列
                        return 2;
                    } else if (ITEM_OTHER == getItemViewType(position)) {
                        //其他商品  //返回3 就是每3个占一个列  就是原本的6列就只占其中的2列
                        return 3;
                    } else {
                        //返回6 就是每6个占一个列  就是原本的6列就只占其中的1列
                        return 6;
                    }
                }
            });
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_HEAD) {
            ...
            //这里为空记得也要设置，不然recycleview可能会导致复用后数据错乱，
            return null;
        }
    }
    ...
    public int getItemViewType(int position) {
        if (position == 0) {
            ...
            //position为2时的bean是BeanImage就返回ITEM_IMAGE这个类型
            return ITEM_IMAGE;
